...
7. **Оркестратор** - Apache Airflow, два DAG: ```demand_forecasting_pipeline``` (daily) и ```demand_forecasting_pipeline_weekly_retraining``` (weekly).
8. **Мониторинг DAG** - сбор статистики запусков, success rate, алерты.
9. **База данных** - SQLite (4 рабочие таблицы: ```sales_history```, ```predictions```, ```pipeline_runs```, ```model_metrics``` + служебная ```schema_version``` для миграций).
...
| **Окружение**	 | Linux / masOS (нативно), Windows (через WSL2), Python venv |
...
### Источники
- **train.csv** - исторические продажи за 2013-2015 гг. (1 017 209 записей до очистки, около 786 тыс. после фильтрации закрытых магазинов и нулевых продаж).
...
3. **Инжиниринг признаков** - ```FeatureEngineer```:            
    - временные (Year, Month, Week, DayOfWeek_sin/cos, Month_sin/cos, IsWeekend, IsMonthStart);        
...
- **Основная модель**: LightGBM с оптимизацией гиперпараметров (Optuna, 10 trials, TimeSeriesSplit, MedianPruner).
...
- **Метрики модели** - при каждом обучении фиксируются timestamp, MAPE, RMSE, параметры.
...
Включая 10 итераций Optuna (на полном наборе данных ~700 тыс. строк).      
**Тестовый период**: последние 20% временной шкалы (все магазины).

---
## Ограничения
- Система обучена и протестирована на данных одной розничной сети (Rossmann); адаптация к другим ритейлерам потребует повторной калибровки признаков и переобучения модели.
- SQLite используется как встроенная СУБД - подходит для прототипа и небольших нагрузок, но не рассчитана на конкурентный доступ в многопользовательской среде.
- Внешние факторы (погода, макроэкономические индикаторы, локальные события) не учитываются в текущем наборе признаков.
- Прогнозирование выполняется в пакетном режиме (batch); real-time inference через REST API не реализован.
- Лаговые признаки требуют непрерывной истории продаж - при длительных пропусках качество прогноза снижается.

---
## Дальнейшее развитие
- Миграция хранилища на PostgreSQL для поддержки конкурентного доступа и масштабирования.
- Реализация REST API (FastAPI) для интеграции с ERP/WMS-системами и получения прогнозов по запросу.
- Обогащение признакового пространства внешними данными: метеосводки, экономические индексы, календари событий.
- Расширение горизонта прогнозирования с однодневного на многошаговый (7, 14, 30 дней).
- Внедрение A/B-тестирования моделей и автоматического отката при деградации метрик.
